#######################################################################
## modified to mark divided error bar, at 08 May 2017, by Dalmin Pai ##
#######################################################################
...
from efficiencyUtils_ErrBar import efficiency_ErrBar
from efficiencyUtils_ErrBar import efficiencyList_ErrBar
import efficiencyUtils_ErrBar as effUtil_ErrBar
...
def EffiGraph1D(effDataList, effMCList, sfList, effDataList2, sfList2, nameout, xAxis = 'pT', yAxis = 'eta'):
...
#    sfMax = 1.02
    ...
    for key in sorted(effDataList.keys()):
        ...
        grBinsEffData2 = effUtil.makeTGraphFromList(effDataList2[key], 'min', 'max')
        grBinsSF2      = effUtil.makeTGraphFromList(sfList2[key]     , 'min', 'max')
        grBinsSF2     .SetMarkerColor( graphColors[igr] )
        grBinsSF2     .SetLineColor(   graphColors[igr] )
        grBinsSF2     .SetLineWidth(2)
        grBinsEffData2.SetMarkerColor( graphColors[igr] )
        grBinsEffData2.SetLineColor(   graphColors[igr] )
        grBinsEffData2.SetLineWidth(2) 
                
        grBinsEffData2.GetHistogram().SetMinimum(effiMin)
        grBinsEffData2.GetHistogram().SetMaximum(effiMax)

        grBinsEffData2.GetHistogram().GetXaxis().SetLimits(xMin,xMax)
        grBinsSF2.GetHistogram()     .GetXaxis().SetLimits(xMin,xMax)
        grBinsSF2.GetHistogram().SetMinimum(sfMin)
        grBinsSF2.GetHistogram().SetMaximum(sfMax)
            
        grBinsEffData2.GetHistogram().GetYaxis().SetRangeUser( effiMin, effiMax )
        ...
        listOfTGraph3.append( grBinsEffData2 )
        listOfTGraph4.append( grBinsSF2 ) 
    ...
    for igr in range(len(listOfTGraph1)+1):
        ...
        listOfTGraph3[use_igr].SetLineColor(graphColors[use_igr])
        listOfTGraph3[use_igr].SetMarkerColor(graphColors[use_igr])
        ...
        listOfTGraph3[use_igr].GetHistogram().SetMinimum(effiMin)
        listOfTGraph3[use_igr].GetHistogram().SetMaximum(effiMax)
        p1.cd()
        listOfTGraph1[use_igr].Draw(option)
        listOfTGraph3[use_igr].Draw(option+" same")
        if not listOfMC[use_igr] is None:
        ...
        listOfTGraph4[use_igr].SetLineColor(graphColors[use_igr])
        listOfTGraph4[use_igr].SetMarkerColor(graphColors[use_igr])
        listOfTGraph4[use_igr].GetHistogram().SetMinimum(sfMin)
        listOfTGraph4[use_igr].GetHistogram().SetMaximum(sfMax)
        if 'pT' in xAxis or 'pt' in xAxis :
            listOfTGraph2[use_igr].GetHistogram().GetXaxis().SetMoreLogLabels()
            listOfTGraph4[use_igr].GetHistogram().GetXaxis().SetMoreLogLabels()
        listOfTGraph2[use_igr].GetHistogram().GetXaxis().SetNoExponent()
        listOfTGraph4[use_igr].GetHistogram().GetXaxis().SetNoExponent()
        listOfTGraph2[use_igr].Draw(option)
        listOfTGraph4[use_igr].Draw(option+" same")
...
def doEGM_SFs(filein, lumi, axis = ['pT','eta'] ):
    ...
    effGraph_ErrBar = efficiencyList_ErrBar()
...
#                           float(numbers[8]),float(numbers[9]),float(numbers[10]), -1 )
            ...
            myeff_ErrBar = efficiency_ErrBar(ptKey,etaKey,
                               float(numbers[4]),float(numbers[5]),float(numbers[6] ),float(numbers[7] ),
                               float(numbers[8]),float(numbers[9]),float(numbers[10]),float(numbers[11]) )
#                           float(numbers[8]),float(numbers[9]),float(numbers[10]), -1 )

            effGraph.addEfficiency(myeff)
            effGraph_ErrBar.addEfficiency(myeff_ErrBar)
...
### massage the numbers a bit
    ...
    effGraph_ErrBar.symmetrizeSystVsEta()
    effGraph_ErrBar.combineSyst()
    ...
    pdfout = nameOutBase + '_egammaPlots_ErrBar.pdf'
    cDummy = rt.TCanvas()
    cDummy.Print( pdfout + "[" )
...
#                              xAxis = axis[1], yAxis = axis[0] )
    ...
    ########################   to divide error bar ( by Dalmin Pai )   ########################
    EffiGraph1D( effGraph.pt_1DGraph_list( False ) , #eff Data
                 None, 
                 effGraph.pt_1DGraph_list( True ) , #SF
                 effGraph_ErrBar.pt_1DGraph_list( False ) , #change uncertainty of eff Data
                 effGraph_ErrBar.pt_1DGraph_list( True ) , #change uncertainty of SF
                 pdfout,
                 xAxis = axis[0], yAxis = axis[1] )
    listOfSF1D = EffiGraph1D( effGraph.eta_1DGraph_list( typeGR =  0 ) , # eff Data
                              effGraph.eta_1DGraph_list( typeGR = -1 ) , # eff MC
                              effGraph.eta_1DGraph_list( typeGR = +1 ) , # SF
                              effGraph_ErrBar.eta_1DGraph_list( typeGR =  0 ) , #change uncertainty of eff Data
                              effGraph_ErrBar.eta_1DGraph_list( typeGR = +1 ) , #change uncertainty of SF
                              pdfout, 
                              xAxis = axis[1], yAxis = axis[0] )
    ###########################################################################################
